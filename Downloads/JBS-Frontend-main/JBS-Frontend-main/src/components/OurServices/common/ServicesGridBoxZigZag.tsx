...
interface SectionBoxZigZagSlideType {
  title: string;
  description: string;
  image: string;
}

interface SectionBoxZigZagSlideProps {
...
const SectionBoxZigZagSlide: React.FC<SectionBoxZigZagSlideProps> = ({
  types,
  sectionTitle,
  sectionDescription,
  height,
  ...
  return (
    <div
      className="mx-auto pt-8 px-4 md:px-64 mb-6 md:h-full mt-10 max-md:-mt-2"
      style={{
        background: "linear-gradient(to bottom, white 50%, #069986 50%)",
      }}
    >
      {/* Section Title */}
      <h3 className="text-3xl 2xl:text-4xl font-semibold text-center max-md:text-left md:px-4 px-0 mb-0 text-[#069986] md:mb-6">
        {sectionTitle}
      </h3>
      {/* Section Description */}
      {sectionDescription && (
      ...
      {/* Carousel */}
      <div ref={splideRef} className="splide myCarousel md:mt-10 mt-2 ">
        <div className="splide__track px-16">
          <ul className="splide__list">
            {types.map((type, index) => (
              <li
                key={index}
                className="splide__slide py-12"
                style={{ marginTop: index % 2 === 0 ? "30px" : "0" }} // Adds margin-top to alternate boxes
              >
                <div
                  className={`p-8 bg-white shadow-xl rounded-2xl rounded-br-[70px] border-t-[3px] border-[1px] min-h-[350px] transition-all ${
                    index >= 0
                      ? "border-[#eee] hover:shadow-xl hover:border-[#069986]"
                      : "border-[#ccc] hover:shadow-2xl"
                  }`}
                >
                  {/* Icon */}
                  ...
                  {/* Description */}
                  <p
                    className={`2xl:mt-1 text-[14px] 2xl:text-[15px] leading-relaxed mt-1 ${
...
export default SectionBoxZigZagSlide;
