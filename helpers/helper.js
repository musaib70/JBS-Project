...
module.exports = {
     ...
     */
    relationModel: (reference) => {
        let models = {
            charge_type: db[process.env.DB].charge_type,
            charge_type_variable: db[process.env.DB].charge_type_variable,
            where_condition: db[process.env.DB].where_condition,
        }
        
        return (models[reference]) ? models[reference] : '';
    },
}